...
sac <- function(environmentalLayers,num_sample=2500,autocorrelationClassDistance=10,autocorrelationMaxDistance=500,autocorrelationSignif=0.05) {
  ...
  space <- sp::spDists(as.matrix(environmentalLayers.r[,c("x","y")]),as.matrix(environmentalLayers.r[,c("x","y")]),longlat=TRUE)
  ...
  for( i in 1:ncol(environmentalLayers.r)) {
      ...
      data <- sp::spDists(as.matrix(environmentalLayers.r[,i]), as.matrix(environmentalLayers.r[,i]),longlat=FALSE)
