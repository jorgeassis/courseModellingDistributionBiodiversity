...
myStudyRegion <- studyRegion(rasterLayers=environmentalLayers, # SpatRaster of layers
                             ...
                             verticalPref=c(-15,-250)) # known vertical distribution
...
weights <- c(model_brt$performance[1, "auc.prg"], model_xgboost$performance[1, "auc.prg"], model_maxent$performance[1, "auc.prg"], model_glm$performance[1, "auc.prg"])
weights

ensembleWeighted <- app(c(prediction_brt, prediction_xgboost, prediction_maxent, prediction_glm), fun = function(x) { weighted.mean(x, w = weights) })

# map the predicted distribution
plot(probabilityRaster, main="Predicted species distribution",col = rev(topo.colors(100)))
plot(ensembleWeighted, main="Predicted species distribution",col = rev(topo.colors(100)))
